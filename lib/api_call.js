...
var parse = function(apiCall, context) {
  ...
  var defaults = util.nestedValue.get(context, 'config.defaults.api_call') || {},
      result = deepInterpolate(apiCall, context.data);
  result = deepArrayMerge(result);
  result = util.deepMerge(defaults, result);
  result = deepInterpolate(result, context.data);
  result = replacePathWithUrl(result);
  return result;
};
exports.parse = parse;
