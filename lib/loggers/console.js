...
module.exports = function(config) {
  return {
    ...
    all: {      
        ...
        output("\n" + stats.failing_tests + "/" + stats.tests + " tests failed in " + stats.suites + " suites. " + stats.api_calls + " API calls with average response time " + Math.round(stats.average_response_time) + "ms\n");
        if (success) {
          output("\nSUCCESS!\n");
        } else {
          output("\nFAILURE!\n");
