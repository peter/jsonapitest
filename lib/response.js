...
var assert = function(assertion, response) {
  ...
  if (assertion.schema) {
    schemaErrors = assertFunctions.schema(selectValue, assertion.schema).error_messages;
    if (schemaErrors.length > 0) {
      errors.push({type: 'schema', select: assertion.select, expected: assertion.schema, actual: selectValue, errors: JSON.stringify(schemaErrors)});
    }
  }

  if (assertion.equal && !assertFunctions.equal(selectValue, assertion.equal)) {
    errors.push({type: 'equal', select: assertion.select, expected: assertion.equal, actual: selectValue});
  }
  if (assertion.not_equal && assertFunctions.equal(selectValue, assertion.not_equal)) {
    errors.push({type: 'not_equal', select: assertion.select, expected: assertion.not_equal, actual: selectValue}); 
  }

  if (assertion.equal_keys && !assertFunctions.equal_keys(selectValue, assertion.equal_keys)) {
    errors.push({type: 'equal_keys', select: assertion.select, expected: assertion.equal_keys, actual: selectValue});
  }

  if (assertion.length && !assertFunctions.length(selectValue, assertion.length)) {
    errors.push({type: "length", select: assertion.select, expected: assertion.length, actual: (selectValue && selectValue.length)});
  }

  if (assertion.contains && !assertFunctions.contains(selectValue, assertion.contains)) {
    errors.push({type: 'contains', select: assertion.select, expected: assertion.contains, actual: selectValue});
  }
  if (assertion.not_contains && assertFunctions.contains(selectValue, assertion.not_contains)) {
