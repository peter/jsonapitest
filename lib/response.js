...
var assert = function(assertion, response) {
  ...
  if (assertion.length && (selectValue.length == null || selectValue.length !== assertion.length)) {
    errors.push({type: "length", select: assertion.select, expected: assertion.length, actual: selectValue.length});
  }
