...
exports.loadModule = function(context, key) {
  ...
  var path = nestedValue.get(context.config, ('modules.' + key)),
      logFunction = context.logger && nestedValue.get(context.logger, 'module.loaded');
  path = path || defaultModulePaths[key];
  if (path == null) return null;
  loadedModules[key] = (key === 'logger' ? require(path)(context.config) : require(path));
